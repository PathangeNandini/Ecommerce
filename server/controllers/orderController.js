...
exports.placeOrder = async (req, res) => {
  try {
    ...
    if (io) {
      ...
      // ✅ Notify restaurant owner dashboard
      io.to(`restaurant:${restaurantId}`).emit('order:placed', payload);
      io.to('owner:all').emit('order:placed', payload);

      // ✅ Notify courier dashboard so new order appears in available deliveries
      io.to('couriers:all').emit('order:placed', payload);
    }
...
/** GET /api/orders/pending — placed orders for this restaurant */
...
/** GET /api/orders/all — ALL orders for this restaurant */
...
exports.getAvailableDeliveries = async (req, res) => {
  try {
    const orders = await Order.find({ status: { $in: ['placed', 'preparing'] }, courierId: null })
      .populate('restaurantId', 'name address')
...
exports.updateStatus = async (req, res) => {
  try {
    ...
    if (io) {
      ...
      // ✅ When a courier accepts, remove from all couriers' available list
      if (status === 'assigned') {
        io.to('couriers:all').emit('order:assigned', { orderId: order._id });
      }
    }
...
exports.getDailyRevenue = async (req, res) => {
  try {
    ...
    res.json({ last7Days, grandTotal, totalOrders, restaurantName: restaurant.name });
  } catch (err) {
    res.status(500).json({ msg: err.message });
  }
};
